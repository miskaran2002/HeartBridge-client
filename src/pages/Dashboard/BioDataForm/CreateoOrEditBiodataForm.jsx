...
const CreateOrEditBiodataForm = ({ onSubmit }) => {
    const { user } = useAuth();
    ...
    // Prefill Firebase user name/email
    useEffect(() => {
        if (user) {
        ...
        >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input {...register('name', { required: true })} className="w-full border p-2 rounded" placeholder="Your Name" />
                </div>

                {/* Image */}
                <div>
                    <label className="block mb-1">Profile Image URL</label>
                    <input {...register('image')} className="w-full border p-2 rounded" placeholder="Image URL or leave blank if uploading" />
                </div>
...
export default CreateOrEditBiodataForm;
